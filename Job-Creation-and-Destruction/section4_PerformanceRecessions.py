...
file_path = "State_Recession_Years.csv"
bds_data = pd.read_csv(file_path)

# Calculate sums and perform division
state_averages = bds_data[bds_data['Period']=='RecessionAvg'].sort_values(
...
# Reorder the rows based on average net job creation rate
...
heatmap_fig.update_layout(
    title="""
    U.S. States Net Job Creation Rate (%) by Recession Years: <br>
    States sorted in order by Average Net Job Creation Rate (Recession Years Only)""",
    xaxis_title='Recession Years',
...
pyo.plot(heatmap_fig, filename='state_heatmap3.html', auto_open=True)
